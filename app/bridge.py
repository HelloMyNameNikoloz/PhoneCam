...
from app.config import DEFAULT_SETTINGS, STABILITY_SETTINGS
...
class PhoneCamBridge:
    ...
    def save_settings(self, settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if merged.get("stabilityMode"):
            merged.update(STABILITY_SETTINGS)
    ...
    def _handle_disconnected_camera(self) -> None:
        ...
        if self.runner.is_running() and not self._selected_ready_device():
            self.runner.stop()
            self.logger.warning("Device disconnected; camera stopped")

    def _handle_autostart(self) -> None:
        if self.settings.get("autoStart") and not self.runner.is_running():
            if self._selected_ready_device():
                ok, message = self.runner.start(self._settings_for_run())
                (self.logger.success if ok else self.logger.error)(message)
    ...
    def _selected_ready_device(self) -> Dict[str, str] | None:
        ...
        if selected:
            return next((d for d in ready if d["id"] == selected), None)
